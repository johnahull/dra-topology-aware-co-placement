...
## Upstream PR Status (as of 2026-09-01)
...
| [#17957](https://github.com/kubevirt/kubevirt/pull/17957) | VM-scoped persistent ResourceClaims for DRA device passthrough | Open (hold) | [#18171](https://github.com/kubevirt/kubevirt/issues/18171) | `/hold` by jean-edouard: VEP must merge first. jean-edouard reviewed (2026-07-31): feature gate guards added for informers/expectations, `persistWhenStopped` implemented with `reservedFor` mechanism. DCO fixed (removed co-author line). verify-go-mod failing. Blocked on VEP #345. Approver: iholder101. **Update 08-27: jean-edouard (COMMENTED) asked to rework/squash the commit history — commits like "chore: regenerate API artifacts after rebase" and "fix: address review feedback" should be folded into the commits that introduce what they fix. `/hold` still in place (no `/unhold`).** Update 08-31: johnahull reworked — rebased on main, squashed to 2 logical commits (`c91768e07f` API + `1ea25506ec` impl), and replied to jean-edouard's 3 threads (feature-gate guarding, `resourceclaims/status` vs `/binding` RBAC, `reservedFor` unit-test coverage). Switched `reservedFor` write from merge-patch to `UpdateStatus` + `RetryOnConflict` (RBAC verb `patch`→`update`). Ball back with jean-edouard to re-review; `/hold` deliberate until VEP #345 merges. |
...
|---|---|---|---|
| [#344](https://github.com/kubevirt/enhancements/issues/344) (tracking) / [#345](https://github.com/kubevirt/enhancements/pull/345) (PR) | VEP-344: VM-Scoped Persistent ResourceClaims | Deferred to KubeVirt 1.11 | Tracking issue #344 (vladikr assigned 08-19) + design PR #345. **Deferred to KubeVirt 1.11** (was being picked up for v1.10). Design debate (Aug 14-19): alaypatel07 flagged manipulating ResourceClaim `status.reservedFor` as risky; johnahull confirmed with wg-device-management the `reservedFor` use case is supported (08-18) and posted an alternative `reservedFor`-based design as an option (08-19); EdDev dislikes the dual VM/VMI API surface, wants a dedicated controller + DRA bi-weekly discussion; enp0s3's manual-deallocation question (08-04) may still be open. Update 08-31: jean-edouard submitted a new COMMENTED review (inline comments only, body empty) — fresh feedback to read/address (he APPROVED 07-31 but LGTM was removed after later changes). Action: read jean-edouard's inline threads (`gh pr view 345 --comments`). Approver: vladikr. |
| [#414](https://github.com/kubevirt/enhancements/pull/414) | VEP-152: Add design to support CPU DRA driver in KubeVirt | Open (PR) | By Sreeja1725. Design consensus reached (2026-08-13): VEP-152 = explicit `cpu.dra` path (low-level), VEP 300 = auto-claim + managed-claim (UX layer). Both target v1.10. vladikr, lyarwood, alaypatel07 aligned. KEP-5304 metadata adopted for CPU topology discovery. Approver: iholder101. **NEEDS johnahull INPUT (you are a reviewer): active multi-maintainer debate 2026-08-24/25 — lyarwood flagged a breaking change not allowed in v1 API, argues the DRA `*ClaimRequest` should be a single struct not a list and that CPU-claim-vs-DRA should be mutually exclusive; vladikr agreed (not a list, defended `dedicatedCpuPlacement` as intent); ffromani/rthallisey/alaypatel07 debating whether `domain.cpu.dra` should exist at all vs hiding DRA behind the KubeVirt API (VEP-300 consumes it). Directly adjacent to your VEP-300. Update 08-26: Sreeja1725 posted a detailed defense of keeping `domain.cpu.dra` — virt-launcher must read the KEP-5304 metadata file to translate the DRA CPU allocation into a libvirt `<cputune>` cpuset; deriving claim/request names from the pod (`container.resources.claims[]` + `pod.spec.resourceClaims[]`) breaks when multiple DRA resources share one pod (claims don't say which is CPU vs GPU/network), and reading pod spec from virt-launcher was a deliberate no. cc'd @vladikr @alaypatel07; ball is with them.** Update 08-26/27: **lyarwood** left 2 inline comments on `vep.md` (08-26): (1) prefers enforcing mutual-exclusion via a CEL expression in the API rather than this design; (2) asked to embed under a `ResourceClaim` (as `NetworkSource` does) instead of a `DRA` field — the list-vs-struct question. **vladikr** (08-27) pointed to his #432 comment and offered Sreeja1725 three alternatives to a spec field: glob the metadata dir filtered by `driverName`; put (claimName, requestName) on VMI status; or have dra-driver-cpu publish `DRA_CPUSET`/`DRA_CPU_NUMA` env via CDI as the fast path. Convergence pending on struct-vs-`ResourceClaim` embedding — johnahull's reviewer call. |
| [#300](https://github.com/kubevirt/enhancements/issues/300) (tracking) / [#432](https://github.com/kubevirt/enhancements/pull/432) (PR) | VEP-300: Managed DRA Claims | Open (PR) | By johnahull, co-author alaypatel07. Filed 2026-08-21. `ManagedClaimProvisioner` CRD maps DeviceClasses; users declare devices (`gpus[]`, `hostDevices[]`, `networks[]`, `cpu.dra`) without hand-authoring ResourceClaims. Built-in topology-aligning provisioner (`policy.kubevirt.io/aligner`) + third-party provisioner support. Claim names derived deterministically from VMI (no status handshake). Builds on VEP-152. size/XL, DCO green. Tracking issue #300. **First human review landed 2026-08-25: alaypatel07 called it "a very well drafted design" (one nit), pinged @rthallisey @lyarwood @vladikr @iholder101. johnahull replied same day (2 comments) — verify the nit is resolved; now awaiting the 4 senior reviewers.** Update 08-27/28: of the 4 pinged, only **vladikr** responded (08-27, long counter-proposal): don't make `domain.cpu.dra` the user-facing API; VEP-152 stays plumbing, VEP-300 owns claim generation, add a `MigrationPolicy`-style `ManagedClaimPolicy` object so users express intent (`dedicatedCpuPlacement`) not mechanism; claim names handed to virt-controller via VMI status, not a spec field. **alaypatel07** pushed back (08-28): removing `domain.cpu.dra` entirely is leaky (pushes user-visible CPU semantics into the provisioner); proposed a concrete `domain.cpu.hostAllocation.groups` API with a worked exclusive-vs-shared vCPU→domxml example. rthallisey/lyarwood/iholder101 silent; the 08-25 nit is not marked resolved (thread shifted to the API debate). **NEEDS johnahull to steer/reconcile vladikr (policy object) vs alaypatel07 (hostAllocation groups).** Update 08-31/09-01: johnahull (08-31) sided with alaypatel07 that vCPU→host-CPU mapping is intent not mechanism, and proposed a trimmed `domain.cpu.hostAllocation.groups` (drop `mappingPolicy`; `exclusive: bool` instead of an `accessMode` enum; `dedicatedCpuPlacement: true` stays as the coarse all-dedicated knob, mutually exclusive with groups). EdDev (09-01, new voice) says the topic is overloaded and wants a generic guest-spec→host-wiring "translation/adapting layer" for ALL devices (admin-defined backend intent), tying it to DRA-networking-with-Multus; alaypatel07 asked EdDev to produce concrete YAML. Now a three-way fork (vladikr policy-object vs alaypatel07/johnahull spec-mapping vs EdDev generic layer); ball with EdDev for YAML. Monitor. **Tracking issue #300 — ACTION (iholder101 08-31): this VEP has no dedicated "VEP Reviewer" on the v1.10 Enhancement Tracking Board (project 22); required to stay "Tracked" for v1.10. As of 09-01 still unassigned (assignee alaypatel07); johnahull did `/remove-lifecycle rotten` 08-28. Assign a VEP Reviewer or #300 drops from v1.10 tracking.** |
| [#424](https://github.com/kubevirt/enhancements/pull/424) | VEP-115: DRA metadata topology and PCIe root complex grouping | Open (PR) | By johnahull. Amends VEP-115 to (1) group devices by PCIe root complex (one `pxb-pcie` per root complex) instead of NUMA-node-only, for accurate peer-to-peer DMA locality on hardware where one NUMA node has multiple root complexes (NPS1); (2) read `resource.kubernetes.io/numaNode` + `resource.kubernetes.io/pcieRoot` from KEP-5304 metadata; (3) add a sysfs topology shim writing KEP-5304 files for devices without metadata (device plugin / DRA drivers that don't publish topology attrs). No new gate (uses existing `PCINUMAAwareTopology`), no API changes. size/L, sig/compute, DCO green. Requests v1.10 milestone. **alaypatel07 left CHANGES_REQUESTED (08-24, "initial thoughts, deeper review later this week"); johnahull posted 3 review replies 08-25. Awaiting alaypatel07's promised deeper review; @mresvanis/@fanzhangio own VEP-115.** **APPROVED 2026-09-01: vladikr `/approve` ("I'll approve this in principle and let @alaypatel07 converge this"); prow flipped to APPROVED, `approved` label on. johnahull pushed an update 09-01. alaypatel07's promised deeper review still never landed — vladikr handed convergence to him. Awaiting alaypatel07 convergence, then merge.** |
...
| PR | Title | State | Issue | Comments |
|---|---|---|---|---|
| [#138732](https://github.com/kubernetes/kubernetes/pull/138732) | Fix CPU manager cpuset reconciler and apply cpuset before container starts | Open | K-2, K-3 | ffromani assigned. Responded 2026-06-22 re: PreCreateContainer. No reply in 53 days. Tests failing. |
...
| [#88](https://github.com/ROCm/k8s-gpu-dra-driver/pull/88) | Auto-partition (dynamic GPU repartitioning) | Open | — | bhatnitish. Feature-gated `AutoPartition`. Repartitions via amd-smi, KEP-4815 mutex counters. Compute-only (no VFIO). Hardware-validated 8×MI300X. CI green. Under heavy review (08-17/19): 8-angle adversarial review (lock held across blocking modprobe, lost reload marker, panic risks). thc1006 flagged checkpoint checksum rollback hazard (`VerifyChecksum` re-marshals decoded struct). #91 now defers NPS `memoryPartition` attr to this PR. Clean CI pass overnight 2026-09-01 (Lint/Test/Build green), but the CHANGES_REQUESTED review (spraveenio 8-angle + thc1006 checksum hazard) still stands — bhatnitish's to address. |
...
| [#153](https://github.com/kubernetes-sigs/dra-driver-cpu/issues/153) | PLANNING: 2026 general roadmap | Open | ffromani's latest timeline. GA target still in flux. Referenced in VEP-152 discussion. |
| [#142](https://github.com/kubernetes-sigs/dra-driver-cpu/issues/142) | PLANNING: Release 0.3.0 | Open | Current release cycle. |
| [#184](https://github.com/kubernetes-sigs/dra-driver-cpu/issues/184) | PLANNING: Release 0.4.0 | Open | Next release planning. |
...
| [#36](https://github.com/kubernetes-sigs/dra-driver-cpu/issues/36) | Absorb the memory allocation feature | Open | Combined CPU+memory driver. VEP-152 and KV-8 resolution depend on this timeline. |
...
| [VEP 115](https://github.com/kubevirt/enhancements/issues/115) | PCIe NUMA Topology Awareness | @mresvanis, @fanzhangio | New | Step 7 | Phase 1 resolved via VEP-10. Phase 2: KEP-6072 merged, helpers merged (#139929). johnahull's design amendment [#424](https://github.com/kubevirt/enhancements/pull/424) carries the live detail; see the KubeVirt Enhancements row (#424). |
| [VEP 152](https://github.com/kubevirt/enhancements/issues/152) | Add support for CPU DRA Driver | @alaypatel07 | New | Step 3 | KubeVirt + DRA CPU driver integration. Feature gate `CPUsWithDRA`. ffromani: GA timeline still in flux (see dra-driver-cpu#153). cpumanager interaction (KV-8/#17708) does not need to wait for driver GA. Design PR [#414](https://github.com/kubevirt/enhancements/pull/414) detail lives in the KubeVirt Enhancements row (#414). |
...
| [VEP 300](https://github.com/kubevirt/enhancements/issues/300) | Device manager like policies for DRA resources | @johnahull | New | Step 4 | KubeVirt-level DRA claim manager with policy-based claim generation. The managed-claim half of the VEP-152/300 split; targets v1.10. Overlaps with topology coordinator's partition DeviceClasses and webhook expansion. Design PR [#432](https://github.com/kubevirt/enhancements/pull/432) and the open VEP-Reviewer action tracked in the KubeVirt Enhancements row (#300/#432). |
