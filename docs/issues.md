...
| [#17957](https://github.com/kubevirt/kubevirt/pull/17957) | VM-scoped persistent ResourceClaims for DRA device passthrough | Open (hold) | [#18171](https://github.com/kubevirt/kubevirt/issues/18171) | Implementation of persistent DRA claims (`persistWhenStopped` via `reservedFor`). `/hold` by jean-edouard until VEP #345 merges. johnahull reworked 08-31: rebased and squashed to 2 commits, switched the `reservedFor` write from merge-patch to `UpdateStatus` + `RetryOnConflict`, and replied to jean-edouard's 3 threads. Ball with jean-edouard to re-review. Approver: iholder101. |
...
| [#424](https://github.com/kubevirt/enhancements/pull/424) | VEP-115: DRA metadata topology and PCIe root complex grouping | Open (PR) | By johnahull. Amends VEP-115 to group devices by PCIe root complex (not NUMA-node-only) for accurate peer-to-peer DMA locality, read `numaNode` + `pcieRoot` from KEP-5304 metadata, and add a sysfs shim for devices lacking metadata. Was APPROVED 2026-09-01 by vladikr, but alaypatel07's promised deeper review landed 09-09 as a scope objection rather than an `/lgtm`: (1) the memory DRA driver still isn't available for use, and (2) CPU requested via VEP-152 could land with kubelet-allocated memory that isn't NUMA-aligned — undercutting the VEP's full-topology benefit. **johnahull posted a revised, reduced-scope plan the same day (09-09 21:57):** for v1.10, split off a bugfix PR that just fixes device-placement ordering to pcieRoot-first-then-NUMA (still sysfs-based), and defer the broader metadata-driven design. **NEEDS johnahull to open that scoped bugfix PR** — that supersedes the original #424 as the v1.10 deliverable; alaypatel07's `/lgtm` is still the gate once the new PR is up. |
