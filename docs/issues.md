...
## Upstream PR Status (as of 2026-09-02)
...
| [#414](https://github.com/kubevirt/enhancements/pull/414) | VEP-152: Add design to support CPU DRA driver in KubeVirt | Open (PR) | By Sreeja1725. VEP-152 = explicit `cpu.dra` low-level path; VEP-300 = managed-claim UX layer (consensus 08-13). **NEEDS johnahull input (you are a reviewer):** open debate on whether `domain.cpu.dra` should be a spec field at all, and list-vs-struct/`ResourceClaim` embedding. vladikr offered alternatives to a spec field (glob metadata by `driverName`, VMI status, or dra-driver-cpu publishing `DRA_CPUSET` env via CDI). Directly adjacent to your VEP-300. Approver: iholder101. |
| [#300](https://github.com/kubevirt/enhancements/issues/300) (tracking) / [#432](https://github.com/kubevirt/enhancements/pull/432) (PR) | VEP-300: Managed DRA Claims | Open (PR) | By johnahull, co-author alaypatel07. `ManagedClaimProvisioner` CRD maps DeviceClasses so users declare devices without hand-authoring ResourceClaims; builds on VEP-152. Three-way design fork on the CPU API: vladikr (`ManagedClaimPolicy` object) vs alaypatel07/johnahull (`domain.cpu.hostAllocation` spec-mapping) vs EdDev (generic guest-spec to host-wiring adapting layer). Live debate 09-01/02: vladikr objects that mixed exclusive/shared guest vCPUs should not be a first-class API (guest OS can't tell which vCPUs are exclusive); alaypatel07 rebuts (user intent, floating stays within a NUMA node, production precedent). **NEEDS johnahull to respond to vladikr's objection and decide whether to fold in EdDev's generic-layer proposal.** **ACTION: no VEP Reviewer assigned on the v1.10 board (project 22); assign one or #300 drops from v1.10 tracking.** |
| [#424](https://github.com/kubevirt/enhancements/pull/424) | VEP-115: DRA metadata topology and PCIe root complex grouping | Open (PR) | By johnahull. Amends VEP-115 to group devices by PCIe root complex (not NUMA-node-only) for accurate peer-to-peer DMA locality, read `numaNode` + `pcieRoot` from KEP-5304 metadata, and add a sysfs shim for devices lacking metadata. No new gate, no API changes; requests v1.10. **APPROVED 2026-09-01 by vladikr `/approve`** (prow APPROVED), but no `/lgtm` yet so prow won't merge. alaypatel07's promised deeper review still hasn't landed (latest is his 08-24 CHANGES_REQUESTED). Ping alaypatel07 for the deeper review + `/lgtm`; that is the only gate to merge. |
...
| [#114](https://github.com/ROCm/k8s-gpu-dra-driver/pull/114) | IOMMUFD support for per-device IOMMU isolation | Open | [#93](https://github.com/ROCm/k8s-gpu-dra-driver/issues/93) | IOMMUConfig API (LegacyOnly/PreferIommuFD), IOMMUFD cdev detection, CDI spec generation, graceful fallback to legacy. Mixed-backend CDI bug fixed during review. Targets `develop`. E2E blocked on KubeVirt libvirt 12.2+. |
