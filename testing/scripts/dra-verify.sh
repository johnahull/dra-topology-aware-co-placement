...
cmd_counters() {
    ...
    slices="$(mktemp)"
    claims="$(mktemp)"
    trap 'rm -f "$slices" "$claims"' RETURN

    if ! kubectl get resourceslices -o json >"$slices" 2>/dev/null; then
        echo "  ${FAIL} unable to read ResourceSlices"
        return 1
    fi
    if ! kubectl get resourceclaims -A -o json >"$claims" 2>/dev/null; then
        echo "  ${FAIL} unable to read ResourceClaims"
        return 1
    fi
    python3 "$SCRIPT_DIR/dra-counters.py" "$slices" "$claims"
}
