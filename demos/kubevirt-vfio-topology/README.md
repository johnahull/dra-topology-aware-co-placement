...
1. **Host topology** — 8 AMD MI355X GPU VFs + 8 Pensando NIC VFs + 8 Mellanox NIC VFs across 2 sockets / 8 NUMA nodes
2. **ResourceSlice summary** — 4 DRA drivers publishing devices with vendor labels
3. **Partition DeviceClasses** — topology coordinator aggregates with pcieRoot + numaNode alignment constraints
4. **VM manifest** — simplified YAML with ResourceClaimTemplate (`eighth count=3`) and no hostDevices
5. **Running VM** — `kubectl get vm,vmi` showing Phase=Running
6. **Claim alignment** — `dra-verify.sh claims` showing 3 independent pcieRoot + numaNode alignment groups
7. **Guest NUMA topology** — SSH into VM, `hw-topology.sh` shows 3 NUMA nodes each with its own GPU + Pensando NIC pair

## Key feature: auto-generated hostDevices

The VMI YAML does **not** specify any `hostDevices`. The topology coordinator's webhook:
1. Reads the `ResourceClaimTemplate` to find the DeviceClass (`eighth`) and count (`3`)
2. Looks up the `PartitionConfig` to discover passthrough device types (GPU, NIC)
3. Auto-generates 6 `hostDevices` entries (3 GPUs + 3 NICs) with correct expanded request names

The user only writes:
```yaml
resourceClaims:
- name: partitions
  resourceClaimTemplateName: vm-3eighth-tpl
```
...
kubectl apply -f demo-vm-3eighth.yaml

# Wait for Running
kubectl get vmi demo-vm-3eighth
...
| `demo-vm-3eighth.yaml` | ResourceClaimTemplate + VMI for 3x eighth (primary demo) |
| `demo-vm-2numa.yaml` | ResourceClaimTemplate + VMI for 2x numa (reference) |
...
- SR-IOV DRA driver with VfConfig global fallback
